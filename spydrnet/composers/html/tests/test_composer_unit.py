...
class TestHTMLComposer(unittest.TestCase):
    ...
    def test_write_json(self):
        ...
        self.composer.elk_json = json.loads(ELKString)
        written_str = self.composer._write_json()
        print(ELKString)
        print(written_str)
        assert ELKString == str(written_str), "JSON format is not compacted"
    ...
    def test_create_top_block(self):
        ...
        output = self.composer._create_top_block()
        self.assertIsInstance(output, OrderedDict)
        self.assertTrue(self.match_output(ELKJsonObj, output),
